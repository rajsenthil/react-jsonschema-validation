...
export default function Home() {
  ...
  useEffect(() => {
    const res = fetch(`/product-schema`)
      ...
      .then((data: any) => {
        ...
        validate('https://json-schema.hyperjump.io/schema')
          .then((valData) => {
            console.log('Compiling the schema...: ', typeof newValidate)
            setValidate2(valData)
            newValidate = valData
            return valData
          })
          .catch((err) => {
  ...
  function onSubmit() {
    ...
    console.log('Onsubmit typeof(validate2): ', typeof validate2)
    ...
    async function validateJson() {
      try {
        valid = await newValidate(productStr)
      } catch (err2: any) {
        console.log('Error while validating: ', err2)
      }
    }
    validateJson()
      .then(() => console.log('Inbetween...'))
      .then((resp) => {
        if (valid.valid && valid.errors.length > 0) {console.log('Validation errors: ', valid.errors)
        return} else {
