...
export default function Home() {
  ...
  useEffect(() => {
    const res = fetch(`/product-schema`)
      ...
      .then((data: any) => {
        ...
        // setValidator(setValidator(validate(JSON.parse(data))))
  ...
  function onSubmit() {
    ...
    // const result = parse(JSON.stringify(product))
    if (!schema) {
      alert("Schema is not defined")
      return
    }    
    addSchema(JSON.parse(schema), 'http://example.com/schemas/string')
    validate('http://example.com/schemas/string').then((dataObj: any)=>{
    const result = dataObj.valid(JSON.stringify(product))

    alert('Validation Results: ' + JSON.stringify(product) + '\n' + result)
    console.log('Validation Results: ' + JSON.stringify(product) + '\n' + result)}
    )
    .catch((err)=>{
      alert('Error \n' + err)
      console.log('Error \n' + err)
